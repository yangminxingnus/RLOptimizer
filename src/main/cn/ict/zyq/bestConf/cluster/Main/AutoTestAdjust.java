...
public class AutoTestAdjust implements ClusterManager{
	...
	private void getProperties(){
		...
		//serverAll is a string consists of all server names separated by comma
		...
		while (!flag && crntTry < maxTry) {
			...
			while(true){
				try {
					...
					if(boolofstop){
						System.out.println("The SUT system is terminated!");
						break;
					}
...
//                System.out.println("performance is : " + performance);
				...
				while(true){
					try {
						...
						if(bool){
							System.out.println("The SUT system is safely stopped");
							break;
						}
					} catch (InterruptedException e) {
						e.printStackTrace();
				...
				//if the cluster is not started correctly, we should try terminate the system
				//	in case any bad things happen
				for(int i = 0; i < numServers; i++)
					cluster.get(i).stopSystem();
				try {
					Thread.sleep(3000);
	...
	public String test(HashMap hmTarget, int num, boolean isInterrupt) {
		String resultString = "";
    	boolean flag = false;
		int crntTry = 0;
		while (!flag && crntTry < maxTry) {
...
//				sutTest.terminateTest();
				sutTest.startTest();
				//这里要等query跑完
				resultString = sutTest.getTestResultString(num, isInterrupt);
//				System.out.println("performance is : " + resultString);
				for(int i = 0; i < numServers; i++)
					cluster.get(i).stopSystem();
